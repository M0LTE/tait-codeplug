...
- **The modem tap-in moves from T13 to T12**, in `audio-and-ptt` and therefore in `pdn-basic` and `pdn-extra`. The two taps are the same tap in every respect the 3DK manual specifies - identical 1.8 ms group delay, identical 14.8 ms from EPTT to full carrier with valid modulation, one shared group-delay-distortion plot, and both past the limiter, the 3 kHz low-pass and pre-emphasis - with one difference: the transmit chain runs `modulator <- T13 <- deviation scaler <- T12`, so T12 goes through the deviation scaler and T13 does not. That scaler is what makes T12's level track the channel spacing, which is the point. `MMA-00011-01` p.112 says to use T12 "if not all the channels that the modem will be communicating on have the same channel spacing or bandwidth". T13 wants 0.29 Vp-p per kHz of deviation whatever the channel is; T12 wants 0.23 on a 25 kHz channel and 0.46 on a 12.5 kHz one. **So one interface built for T12 is correct at both spacings and one built for T13 is correct at exactly one.**
- **This is a calibration change and you have to read it with your interface's resistors in hand.** Moving the tap on an interface whose divider was sized for T13 gains about 2 dB on a 25 kHz channel and loses about 4 dB on a 12.5 kHz one. The tap and the divider are one decision, not two. pdn-soundmodem's `docs/hardware/tait-tm8100-cm108.md` now documents two variants and the matching resistor values; variant A is this tap. If you are running a T13-sized interface on a narrowband channel and you re-apply a profile, set the tap back with `patch <port> txtap T13` or change the resistors.
- **How this was found**, for the record: an interface built to the documented 12.5 kHz values was measured on air on a radio set to 25 kHz and delivered 1.800 kHz peak deviation against a 5.0 kHz class - 36 % - with nothing in software able to recover it. The design was right and the channel spacing it was applied to was not.
- **`pdn-internal` deliberately stays on T13**, and now says so in code rather than getting there by accident of ordering. The internal options board is a different interface with a different divider designed by somebody else, and re-calibrating it blind is not a change to make on inference.
- **Confirmed on two radios before this shipped.** Both TM8110s on the bench were programmed with `patch txtap T12` and the deviation measured off air with an SDRplay RSP1: the radio accepts the byte, the tap moves, and the sensitivity changes by **+2.12 dB measured against +2.01 dB predicted**. So the encoding and Tait's auto-scaling figure both hold to a tenth of a decibel, and the one inference this change rested on is now a measurement.
- **One byte of the profile is still not CPS-validated, and the tests say so.** Every constant the profile tests assert against is a byte-exact capture of what the CPS writes, which is what makes them worth having. The tap-in byte is now 0x38 rather than 0x3A by the documented `payload[11] = 0x20 | (node << 1)` encoding - which is pinned against a CPS save of T13, and against the literal node numbering the field uses for T3, T5 and T8, but has not itself been read off a save of a T12 configuration. The fixture and the test both record that. Take a CPS save of a T12 configuration and it becomes a byte-exact target again.
